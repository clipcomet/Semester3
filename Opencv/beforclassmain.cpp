...
Mat imgHSV;
 ...
 void dump()
 {
     /*
      ...
      Mat convertRGBHSV(Mat imgOriginal) // outputs imgHSV
      {
          cvtColor(imgOriginal, imgHSV, COLOR_BGR2HSV); //Convert the captured frame from RGB to HSV
          return imgHSV;
      ...
      Mat Thresholded(Mat imgHSV) // sliders and return a Threshold img
      {
          int iLowH = 0;
          int iHighH = 179;
      ...
      Mat erodedilate(Mat imgThresholded) // tryes to make picture easter to handle
      {
          //(remove small objects from the foreground "or trying to :P)
          erode(imgThresholded, imgThresholded, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) );
          dilate( imgThresholded, imgThresholded, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) );
      ...
      Point findCenter(Mat imgThresholded) //find Center of Threshold img
      {
          Moments mu = moments(imgThresholded, true);
          Point center;
          center.x = mu.m10 / mu.m00;
      ...
      Mat printcenter (Mat imgOriginal,Point center) // adds circle to center
      {
          circle(imgOriginal, center, 4, Scalar(255,0,255));
         return imgOriginal;
      ...
      Mat printtextcenter (Mat imgOriginal,Point center) // adds text to center
      {
      ...
      Mat vectoroutline (Mat imgThresholded) // draws a vector line around objects
      {
         vector<vector<Point> > contours; // vektor points for outline of object
         findContours(imgThresholded,contours, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0)); //make points from
         drawContours(imgThresholded,contours,-1,(255,255,0),3); //send points too
         return imgThresholded;
      }

      void imShow (Mat img,String name) // img to show and name of Window its showen in
      {
         imshow(name, img);
      }

      void allInOne (Mat imgOriginal)
      {
         vector<vector<Point> > contours; // vektor points for outline of object
         vector<vector<Point> >canny;
         // HSV color range
